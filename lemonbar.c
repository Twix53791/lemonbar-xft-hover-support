...
monitor_new (int x, int y, int width, int height)
{
    ...
    (const uint32_t []) {
        bgc.v, bgc.v, dock, events, colormap
    });
...
main (int argc, char **argv)
{
    ...
    while ((ch = getopt(argc, argv, "hg:bdf:H:a:pu:B:F:U:n:o:")) != -1) {
        switch (ch) {
            case 'h':
                printf ("lemonbar version %s patched with XFT support and supporting enter/leave events (hover).\n", VERSION);
                printf ("usage: %s [-h | -g | -b | -d | -f | -H | -a | -p | -n | -u | -B | -F]\n"
                        "\t-h Show this help\n"
                        "\t-g Set the bar geometry {width}x{height}+{xoffset}+{yoffset}\n"
                        "\t-b Put the bar at the bottom of the screen\n"
                        "\t-d Force docking (use this if your WM isn't EWMH compliant)\n"
                        "\t-f Set the font name to use\n"
                        "\t-H Execute commands on enter and leave window events (hover). -H \"cmd 1:cmd 2\"\n"
            ...
            case 'H': entercmd = strtok(optarg, hdelim); leavecmd = strtok(NULL, hdelim); break;
                        ...
                        case XCB_LEAVE_NOTIFY:
                            {
                                ...
                                if (entered && entercmd != NULL && strcmp( entercmd, "null") != 0)
                                    system (entercmd);
                                else if (!entered && leavecmd != NULL)
                                    system (leavecmd);
                            }
                            break;
                    }
