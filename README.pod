...
Enable the execution of commands on mouse enter/leave window events (hover). Two commands can be given, separated by ":". The first one
will be execute when the mouse will enter the lemonbar window (set to "null" to disable it), when the (optional) second one will be executed when the mouse will leave the window.
